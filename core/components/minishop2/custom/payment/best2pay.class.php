...
class Best2pay extends msPaymentHandler implements msPaymentInterface {
	...
	private function getURL($path) {
		if ($this->modx->getOption('setting_ms2_payment_best2pay_test_mode')) {
			return 'https://test.best2pay.net' . $path;
		} else {
			return 'https://pay.best2pay.net' . $path;
		}
	}
	...
	public function getBest2PayLink(msOrder $order) {
		...
		$tax = $this->modx->getOption('setting_ms2_payment_best2pay_tax');

		switch ($this->modx->getOption('setting_ms2_payment_best2pay_currency', null, 'руб')) {
			case 'евро':
				$currency = '978';
				break;
			case 'доллар':
		...
		switch($this->modx->getOption('setting_ms2_payment_best2pay_type')) {
			case '2':
